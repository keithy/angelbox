...
docker exec -t -i  target apt-get -y install libyaml-dev git upzip

URL_COMPOSER_INSTALLER="https://getcomposer.org/installer"
docker exec -t -i  target sh -c "curl -sS $URL_COMPOSER_INSTALLER | php -- --install-dir=/usr/bin --filename=composer"
...
docker commit target registry.gitlab.com/keithy/angelbox/php_apache:php7_pdo_mysqli_yaml_composer_xdebug
docker push registry.gitlab.com/keithy/angelbox/php_apache:php7_pdo_mysqli_yaml_composer_xdebug

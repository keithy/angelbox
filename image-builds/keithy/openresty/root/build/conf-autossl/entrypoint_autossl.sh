...
# as an alternative, allowing the original entrypoint.sh to still be used. Thus autossl
# can be enabled and disabled by including/omitting /hub/openresty/openresty/+autossl.yml
# let's remove default open resty configuration, we'll conditionally add modified version
